...
from src.config import (
    MODEL_PATH, META_PATH, MAX_OUTPUT_LEN,
    SOS_TOKEN, EOS_TOKEN, DATA_PATH, UNK_TOKEN
)
from src.data_utils import normalize_text, encode_text
from src.model import Seq2SeqTranslator
...
def translate(text: str, model=None, char2idx=None, idx2char=None) -> str:
    ...
    src = torch.tensor(source_idx).unsqueeze(0)

    tgt_idx = [char2idx[SOS_TOKEN]]
    ...
    with torch.no_grad():
        for _ in range(MAX_OUTPUT_LEN):
            ...
            if idx2char[next_token] == EOS_TOKEN:
                break

            tgt_idx.append(next_token)

    result = "".join(
        idx2char[i] for i in tgt_idx[1:]
        if idx2char[i] not in {SOS_TOKEN, UNK_TOKEN, "<PAD>"}
    )

    return result.strip() if result else "번역 실패"