...
async def process(question: str, speak: bool) -> AskResponse:
    ...
    sources = [SourceItem(source=c['source'], chunk_id=c['chunk_id'], score=round(c['score'],4), preview=c['text'][:240]) for c in contexts]  # 검색 결과를 응답 구조로 변환합니다.
...
@app.get('/api/health')
async def health():
    return {'status':'ok','llm_mode':llm.mode,'llm_load_error':llm.load_error,'model_path':str(settings.FINETUNED_MODEL_PATH),'whisper_model':settings.WHISPER_MODEL_SIZE,'embedding_model':settings.EMBEDDING_MODEL_NAME,'tts_provider':settings.TTS_PROVIDER}  # 주요 상태와 모델 경로 및 마지막 로딩 오류를 반환합니다.
