"""
한국어 고객 상담용 Supervised Fine-Tuning 데이터셋 생성 코드입니다.
...
# JSON Lines 파일을 생성하기 위해 파이썬 표준 json 모듈을 가져옵니다.
import json

# 데이터의 순서를 무작위로 섞고 학습/검증 데이터를 분리하기 위해 사용합니다.
import random

# 운영체제와 관계없이 안전하게 파일 경로를 처리하기 위해 Path를 가져옵니다.
from pathlib import Path


# 현재 파이썬 파일이 존재하는 폴더를 프로젝트 기준 경로로 지정합니다.
BASE_DIR = Path(__file__).resolve().parent

# 학습 데이터 파일을 저장할 data 폴더 경로를 지정합니다.
DATA_DIR = BASE_DIR / "data"

# 학습 데이터가 저장될 JSONL 파일 경로입니다.
TRAIN_FILE = DATA_DIR / "train.jsonl"

# 검증 데이터가 저장될 JSONL 파일 경로입니다.
VALID_FILE = DATA_DIR / "valid.jsonl"

# 데이터 분할 결과를 반복 실행해도 동일하게 유지하기 위한 난수 시드입니다.
RANDOM_SEED = 42


# 모든 학습 샘플에 공통으로 적용할 시스템 지시문입니다.
SYSTEM_PROMPT = """
...
# SFT 실습에 사용할 원본 질문과 정답 데이터를 정의합니다.
RAW_SAMPLES = [
    {
...
def convert_to_messages(sample: dict) -> dict:
    """
    user/assistant 구조의 원본 샘플을 대화형 messages 구조로 변환합니다.
    """

    # SFTTrainer가 이해할 수 있도록 system, user, assistant 순서의 메시지를 반환합니다.
    return {
        "messages": [
...
def validate_sample(sample: dict) -> None:
    """
    학습 데이터 한 건의 구조와 필수 값을 검사합니다.
    """

    # 최상위 데이터에 messages 키가 있는지 검사합니다.
...
def save_jsonl(samples: list[dict], file_path: Path) -> None:
    """
    데이터 목록을 한 줄에 JSON 객체 하나가 들어가는 JSONL 형식으로 저장합니다.
    """
    ...
    with file_path.open("w", encoding="utf-8") as file:
        ...
        for sample in samples:
            # ensure_ascii=False를 사용하여 한글이 유니코드 이스케이프가 아닌 원문으로 저장되게 합니다.
            json_line = json.dumps(sample, ensure_ascii=False)
            # JSON 문자열 뒤에 줄바꿈을 추가하여 JSONL 한 행을 완성합니다.
            file.write(json_line + "\n")


def main() -> None:
    """
    전체 데이터를 변환하고 검증한 뒤 학습용과 검증용으로 분할하여 저장합니다.
    """

    # data 폴더가 없으면 상위 폴더까지 포함하여 생성합니다.
    DATA_DIR.mkdir(parents=True, exist_ok=True)

    # 모든 원본 샘플을 messages 형식으로 변환합니다.
    converted_samples = [convert_to_messages(sample) for sample in RAW_SAMPLES]

    # 변환된 모든 데이터의 구조를 검사합니다.
    for sample in converted_samples:
        validate_sample(sample)

    # 고정된 시드를 사용하는 별도 난수 생성기를 만듭니다.
    random_generator = random.Random(RANDOM_SEED)

    # 원본 순서를 보존하기 위해 데이터 목록을 복사합니다.
    shuffled_samples = converted_samples.copy()

    # 학습과 검증 데이터가 섞이도록 샘플 순서를 무작위로 변경합니다.
    random_generator.shuffle(shuffled_samples)

    # 전체의 80% 지점을 학습/검증 분할 위치로 계산합니다.
    split_index = int(len(shuffled_samples) * 0.8)

    # 앞쪽 80%를 학습 데이터로 사용합니다.
    train_samples = shuffled_samples[:split_index]

    # 나머지 20%를 검증 데이터로 사용합니다.
    valid_samples = shuffled_samples[split_index:]

    # 학습 데이터를 train.jsonl 파일에 저장합니다.
    save_jsonl(train_samples, TRAIN_FILE)

    # 검증 데이터를 valid.jsonl 파일에 저장합니다.
    save_jsonl(valid_samples, VALID_FILE)

    # 데이터 생성 결과를 화면에 출력합니다.
    print("=" * 70)
